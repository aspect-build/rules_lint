...
@test "should produce an applicable SQLFluff fix patch" {
	run bazel build --config=lint --output_groups=rules_lint_patch --@aspect_rules_lint//lint:fix //src:query_template //src:mixed_configs_with_target_data
	assert_success
	...
	run test ! -s bazel-bin/src/mixed_configs_with_target_data.AspectRulesLintSQLFluff.patch
	assert_success
}
