...
'map(
  {
    "key": .tag_name,
    "value": .assets
        | map(select((.name | contains("ty-")) and (.name | contains("sha256") | not) ))
        | map({
            "key": .name | capture("ty-(?<platform>.*)\\.(tar\\.gz|zip)") | .platform,
...
CURRENT=$(mktemp)
python -c "import json; exec(open('$SCRIPT_DIR/ty_versions.bzl').read()); print(json.dumps(TY_VERSIONS))" > $CURRENT
OUT=$(mktemp)
...
(
  echo '"This file is automatically updated by mirror_ty.sh"'
  echo -n "TY_VERSIONS = "
  cat $OUT
)>$SCRIPT_DIR/ty_versions.bzl
