...
// buildContext / filterSourceFilesForTags / isCgoProcessed are vendored from
// the rules_go gopackagesdriver build_context.go. They are part of the "read
// half" the vendored flatpackage.go (FilterFilesForBuildTags) and
// packageregistry.go (ResolvePaths) depend on.
...
func filterSourceFilesForTags(files []string) []string {
	...
	for _, f := range files {
		...
		// MatchFile filters out anything without a file extension. In the case
		// of CompiledGoFiles (in particular cgo processed files from the
		// cache), we want them. We also want to keep cgo processed files with
...
func isCgoProcessed(fileName string) bool {
	// go/tools/builders/cgo2.go generates cgo processed code that are named
	// *.cgo1.go, _cgo_gotypes.go, and _cgo_imports.go.
