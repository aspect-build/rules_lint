...
def _write_assert(ctx, files):
    ...
    for f in files.to_list():
        if f.path.endswith(".txt"):
            ...
            fail("rules_lint_output output group contains unrecognized file extension: ", f.path)
...
def _test_impl(ctx):
    ...
    asserts = [_write_assert(ctx, src[OutputGroupInfo].rules_lint_output) for src in ctx.attr.srcs]
    ...
    return [DefaultInfo(
        executable = bin,
        runfiles = ctx.runfiles([ctx.file._runfiles_lib], transitive_files = depset(transitive = [src[OutputGroupInfo].rules_lint_output for src in ctx.attr.srcs])),
    )]
